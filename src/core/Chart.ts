...
import type {
  ChartOptions, AxisOptions, SeriesOptions, SeriesUpdateData,
  ZoomOptions, CursorOptions, ChartEventMap, Bounds,
} from '../types';
...
class ChartImpl implements Chart {
  ...
  constructor(options: ChartOptions) {
    const originalCanvas = options.canvas;
    ...
    const bgColor = parseColor(options.background ?? this.theme.backgroundColor);
    ...
    this.xAxisOptions = options.xAxis ?? { scale: 'linear', auto: true };
    this.yAxisOptions = options.yAxis ?? { scale: 'linear', auto: true };
    this.xScale = this.xAxisOptions.scale === 'log' ? new LogScale() : new LinearScale();
    this.yScale = this.yAxisOptions.scale === 'log' ? new LogScale() : new LinearScale();

    // Create DOM structure
    this.container = this.createContainer(originalCanvas);
    ...
    originalCanvas.parentElement?.replaceChild(this.container, originalCanvas);
  ...
  private createContainer(original: HTMLCanvasElement): HTMLDivElement {
    const div = document.createElement('div');
    div.style.cssText = `position:relative;width:${original.style.width || '100%'};height:${original.style.height || '100%'}`;
    return div;
  }

  private createCanvas(type: 'webgl' | 'overlay'): HTMLCanvasElement {
    const c = document.createElement('canvas');
    c.style.cssText = type === 'webgl'
      ? `position:absolute;top:${MARGINS.top}px;left:${MARGINS.left}px;width:calc(100% - ${MARGINS.left + MARGINS.right}px);height:calc(100% - ${MARGINS.top + MARGINS.bottom}px)`
  ...
  updateSeries(id: string, data: SeriesUpdateData): void {
    ...
    s.updateData(data);
    this.updateSeriesBuffer(s);
    if (data.append && (this.xAxisOptions.auto || this.yAxisOptions.auto)) this.autoScale();
    this.requestRender();
  }
  ...
  zoom(options: ZoomOptions): void {
    if (options.x) { this.viewBounds.xMin = options.x[0]; this.viewBounds.xMax = options.x[1]; }
    if (options.y) { this.viewBounds.yMin = options.y[0]; this.viewBounds.yMax = options.y[1]; }
  ...
  pan(deltaX: number, deltaY: number): void {
    ...
    const dx = (deltaX / pa.width) * (this.viewBounds.xMax - this.viewBounds.xMin);
    const dy = (deltaY / pa.height) * (this.viewBounds.yMax - this.viewBounds.yMin);
    this.viewBounds.xMin -= dx; this.viewBounds.xMax -= dx;
    this.viewBounds.yMin += dy; this.viewBounds.yMax += dy;
  ...
  public autoScale(): void {
    ...
    this.series.forEach((s) => {
      const b = s.getBounds();
      if (b) { xMin = Math.min(xMin, b.xMin); xMax = Math.max(xMax, b.xMax); yMin = Math.min(yMin, b.yMin); yMax = Math.max(yMax, b.yMax); }
    });
    const xPad = (xMax - xMin) * 0.05 || 0.1, yPad = (yMax - yMin) * 0.05 || 0.1;
    if (this.xAxisOptions.auto) { this.viewBounds.xMin = xMin - xPad; this.viewBounds.xMax = xMax + xPad; }
    if (this.yAxisOptions.auto) { this.viewBounds.yMin = yMin - yPad; this.viewBounds.yMax = yMax + yPad; }
  }
  ...
  disableCursor(): void { this.cursorOptions = null; this.cursorPosition = null; }
  ...
  render(): void {
    ...
    this.series.forEach((s) => {
      ...
      if (buf) seriesData.push({ id: s.getId(), buffer: buf, count: s.getPointCount(), style: s.getStyle(), visible: s.isVisible(), type: s.getType() });
    });
    this.renderer.render(seriesData, { bounds: this.viewBounds, backgroundColor: this.backgroundColor });

    // Overlay render
    const rect = this.container.getBoundingClientRect();
    const plotArea = this.getPlotArea();
    this.overlay.clear(rect.width, rect.height);
    this.overlay.drawGrid(plotArea, this.viewBounds, this.xScale, this.yScale);
    this.overlay.drawXAxis(plotArea, this.viewBounds, this.xScale, this.xAxisOptions.label);
    this.overlay.drawYAxis(plotArea, this.viewBounds, this.yScale, this.yAxisOptions.label);
    ...
    if (this.cursorOptions?.enabled && this.cursorPosition) {
      const cursor: CursorState = {
        enabled: true, x: this.cursorPosition.x, y: this.cursorPosition.y,
        crosshair: this.cursorOptions.crosshair ?? false,
        tooltipText: this.cursorOptions.formatter
          ? this.cursorOptions.formatter(this.pixelToDataX(this.cursorPosition.x), this.pixelToDataY(this.cursorPosition.y), '')
          : `X: ${this.pixelToDataX(this.cursorPosition.x).toFixed(3)}\nY: ${this.pixelToDataY(this.cursorPosition.y).toExponential(2)}`,
      };
      this.overlay.drawCursor(plotArea, cursor);
    }

    const frameTime = performance.now() - start;
    this.events.emit('render', { fps: frameTime > 0 ? 1000 / frameTime : 999, frameTime });
  }
  ...
  private handleBoxZoom(rect: { x: number; y: number; width: number; height: number } | null): void {
    if (rect === null) {
      if (this.selectionRect && this.selectionRect.width > 5 && this.selectionRect.height > 5) {
        ...
        const xMaxNorm = (this.selectionRect.x + this.selectionRect.width - plotArea.x) / plotArea.width;
        const yMaxNorm = 1 - (this.selectionRect.y - plotArea.y) / plotArea.height;
        const yMinNorm = 1 - (this.selectionRect.y + this.selectionRect.height - plotArea.y) / plotArea.height;
...
export function createChart(options: ChartOptions): Chart { return new ChartImpl(options); }
export { ChartOptions };
