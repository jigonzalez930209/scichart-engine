...
const ICONS = {
  ...
  LINE: `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="M18.7 8l-5.1 5.2-2.8-2.7L7 14.3"></path></svg>`,
  SCATTER: `<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><circle cx="7" cy="14" r="2"></circle><circle cx="11" cy="10" r="2"></circle><circle cx="15" cy="13" r="2"></circle><circle cx="19" cy="8" r="2"></circle><path d="M3 3v18h18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>`,
  ...
  SMOOTH: `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12c.5 0 .9-.3 1.2-.7l1.6-2.6c.3-.4.7-.7 1.2-.7h2c.5 0 .9.3 1.2.7l1.6 2.6c.3.4.7.7 1.2.7h2c.5 0 .9-.3 1.2-.7l1.6-2.6c.3-.4.7-.7 1.2-.7h2"></path></svg>`,
  AUTOSCALE: `<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg>`,
...
export class ChartControls {
  ...
  private updateToolbarStyle(): void {
    ...
    const bg = isDark ? 'rgba(30, 30, 45, 0.4)' : 'rgba(255, 255, 255, 0.6)';
    const borderColor = isDark ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)';
    const shadow = isDark ? '0 2px 8px rgba(0, 0, 0, 0.4)' : '0 2px 8px rgba(0, 0, 0, 0.1)';
  ...
  private createButtons(): void {
    // Pan Mode
    this.createButton(ICONS.PAN, 'Pan Mode', () => {
    ...
    // Reset Zoom
    this.createButton(ICONS.RESET, 'Reset Zoom', () => this.callbacks.onResetZoom(), 'reset');

    // Auto Scale
    this.createButton(ICONS.AUTOSCALE, 'Auto Scale', () => this.callbacks.onAutoScale(), 'autoscale');

    // Separator
    const sep = document.createElement('div');
    const isDark = this.theme.name.toLowerCase().includes('dark') || this.theme.name.toLowerCase().includes('midnight');
    sep.style.cssText = `width: 1px; height: 16px; background: ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}; margin: 0 4px;`;
    this.toolbar.appendChild(sep);

    // Type Switcher
    this.createButton(ICONS.LINE, 'Toggle Line/Scatter/Both', () => {
      const types: ('line' | 'scatter' | 'line+scatter')[] = ['line', 'scatter', 'line+scatter'];
    ...
    // Smoothing
    this.createButton(ICONS.SMOOTH, 'Automated Smoothing', () => {
    ...
    sep2.style.cssText = `width: 1px; height: 16px; background: ${isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'}; margin: 0 4px;`;
    this.toolbar.appendChild(sep2);

    // Export Image
    this.createButton(ICONS.EXPORT, 'Export as PNG', () => this.callbacks.onExport(), 'export');

    this.updateButtonStates();
  }

  private createButton(svg: string, title: string, onClick: () => void, id: string): HTMLButtonElement {
    ...
    const isDark = this.theme.name.toLowerCase().includes('dark') || this.theme.name.toLowerCase().includes('midnight');
    const color = this.theme.xAxis.labelColor;
  ...
  private updateButtonStates(): void {
    ...
    const smoothActiveColor = '#ff0055';
    const normalColor = this.theme.xAxis.labelColor;
    ...
    buttons.forEach((btn: HTMLButtonElement) => {
      ...
      if (id === 'pan') {
        ...
        btn.style.opacity = isHover || this.isPanMode ? '1' : '0.7';
      } else if (id === 'smooth') {
        btn.style.color = this.isSmoothing ? smoothActiveColor : normalColor;
        btn.style.opacity = isHover || this.isSmoothing ? '1' : '0.7';
      } else if (id === 'type') {
        btn.innerHTML = this.currentType === 'line' ? ICONS.LINE : (this.currentType === 'scatter' ? ICONS.SCATTER : ICONS.BOTH);
        btn.style.color = normalColor;
        btn.style.opacity = isHover ? '1' : '1';
      } else if (id === 'reset' || id === 'autoscale' || id === 'export') {
        btn.style.color = normalColor;
        btn.style.opacity = isHover ? '1' : '0.7';
      }
